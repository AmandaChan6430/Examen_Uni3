...
def bg_user(event):
    ...
    if texto_actual == "Usuario":
        nombre_admin.delete(0, END)
        nombre_admin.config(foreground='black')
    elif texto_actual == "":
        nombre_admin.insert(0, "Usuario")
        nombre_admin.config(foreground='gray63')

def bg_user2(event):
    texto_actual = nombre_admin2.get()
    if texto_actual == "Usuario":
        nombre_admin2.delete(0, END)
        nombre_admin2.config(foreground='black')
    elif texto_actual == "":
...
def bg_contra(event):
    ...
    if texto_actual == "Contraseña":
        contra_admin.delete(0, END)
        contra_admin.config(foreground='black', show="●")
    elif texto_actual == "":
...
def bg_contra2(event):
    ...
    if texto_actual == "Contraseña":
        contra_admin2.delete(0, END)
        contra_admin2.config(foreground='black', show="●")
    elif texto_actual == "":
        contra_admin2.insert(0, "Contraseña")
        contra_admin2.config(foreground='gray63', show=())

def bg_corr(event):
    texto_actual = cargo_admin.get()
...
def validacionC(valor):
    ...
    valid = False
    # El método fechone de la clase Cursor retorna una tupla con la fila de la tabla que coincide con el código ingrexsado o retorna 'None':
...
def verificar_clien(event):
    ...
    for i in letra:
        if i not in 'qwertyuiopasdfghjklzxcvbnmñ QWERTYUIOPASDFGHJKLZXCVBNM':
            name.delete(letra.index(i), letra.index(i)+1)

def verificar_masc(event):
    letra = namemasc.get()
    for i in letra:
        if i not in 'qwertyuiopasdfghjklzxcvbnmñ QWERTYUIOPASDFGHJKLZXCVBNM':
            namemasc.delete(letra.index(i), letra.index(i)+1)

def verificar_raza(event):
    letra = raza.get()
    for i in letra:
        if i not in 'qwertyuiopasdfghjklzxcvbnmñ QWERTYUIOPASDFGHJKLZXCVBNM':
            raza.delete(letra.index(i), letra.index(i)+1)

def verificar_ag(event):
    letra = age.get()
    for i in letra:
        if i not in '1234567890':
            age.delete(letra.index(i), letra.index(i)+1)
...
def creacion_admin():
    ...
    global usuario_log
    usuario_log = nombre_entry
    global contra_entryg 
    contra_entryg = contra_entry
    global cargo_admin
